...
- `ToCore`(UI→core):`Connect{id,params,pty}`、`Input{id,data}`、`Resize{id,cols,rows}`、`Scroll{id,delta}`、`Sftp{id,request_id,req}`、`StartMonitor{id}`、`Operations{id,operation_id,request}`、`OpenContainerTerminal{id,exec_id,container_id,pty}`、`ContainerInput{id,exec_id,data}`、`ContainerResize{id,exec_id,cols,rows}`、`ContainerScroll{id,exec_id,delta}`、`CloseContainerTerminal{id,exec_id}`、`SetupShellIntegration{id}`、`AuthResponse{id,generation,response}`、`Disconnect{id}`。认证答案必须带产生挑战的连接代次，旧弹窗答案不得进入重连会话。
- `FromCore`(core→UI):`Connected`、`Render{snapshot}`、`Title`、`Cwd{path}`、`Bell`、`SftpListing{request_id,path,entries}`、`SftpProgress{request_id,name,transferred,total}`、`SftpDone{request_id,op,path}`、`SftpError{request_id,message}`、`SftpStopped`、`Monitor{stats}`、`MonitorStopped`、`MonitorError{message}`、`OperationResult{operation_id,domain,result}`、`OperationFailed{operation_id,domain,error}`、`ExecStarted{exec_id,container_id}`、`ExecRender{exec_id,snapshot}`、`ExecClosed{exec_id,error}`、`AuthChallenge{id,generation,challenge}`、`HostKeyPending{id}`、`Closed{error}`。SFTP 请求级事件必须回传 UI 分配的 `SftpRequestId`；`OperationResult`/`OperationFailed` 必须携带 UI 全局分配且不复用的 `OperationId`；容器事件必须匹配 UI 分配且不复用的 `ExecId`；`SftpStopped` 与 `Closed` 保持会话级语义。跨任务的内部事件信封还必须携带连接代次，core 只转发当前代次事件，并在转发当前代次 `Closed` 后移除会话句柄，避免同一 `SessionId` 重连时旧任务污染新连接。`Cwd` 由 `session.rs` 扫描 PTY 原始字节解析 OSC 7(`ESC]7;file://host/path`)得到,写入 `SessionState.terminal_cwd`,供 SFTP「跟随终端目录」使用。
...
- **辅助能力闭环原则**:SFTP、Monitor 等辅助能力必须在成功、失败、超时或会话关闭时收敛;core 路由失败和子通道打开失败要返回对应 `*Error` 事件,子任务正常停止返回 `*Stopped` 事件,UI state 保存 `loading/error/data`。SFTP UI 只接受仍在 pending 集合中的请求级进度/完成/失败事件，关闭、超时和重连会清空 pending 及完成/失败队列，防止旧事件污染新连接。唯一例外是 `SetupShellIntegration`:它没有 loading/pending 状态,失效时 UI 侧的输入推断兜底仍然工作,因此路由失败只记日志,不新增 `FromCore` 错误事件,以免把尽力而为的增强写成用户可见故障。
- **远程运维只读查询**:`OperationsRequest::Refresh` 只能选择 Services/Processes/Network/Docker 等 core 内固定 allowlist 领域，UI 搜索或排序不得进入远端命令。每次查询新开无 PTY exec channel，与交互终端、SFTP、Monitor 隔离；开通道 8 秒、执行 12 秒、stdout 8 MiB、stderr 64 KiB。服务解析的 `ServiceSummary` 保留 `load_state`、`active_state`、`sub_state` 与描述，避免把 systemd 的 SUB 列混入描述。`SessionState.operations` 只保存最后成功快照、当前 `OperationId` 和局部错误；失败不清空旧快照，失败状态不自动重试，用户手动刷新后才重新请求；`Closed` 与重连清理全部运维瞬态，迟到 ID 不得覆盖较新的请求。事件日志仅记录类型、会话和请求标识，不得输出原始查询内容。
- **独立容器终端**:`OpenContainerTerminal` 只携带已校验的 Docker 名称/ID 与 PTY 尺寸，core 固定执行 `docker exec -it <id> /bin/sh`，不接受 UI shell 文本。每个容器终端独占 SSH session channel、PTY 和 `TermEngine`，用独立 `ExecId` 区分生命周期；只有 PTY/exec 成功确认后发送 `ExecStarted`，输出发送 `ExecRender`，远端退出、用户关闭、容器替换或宿主会话断开统一发送一次 `ExecClosed`。容器终端是当前运维中心唯一的交互式远端写入例外；查询主动取消和独立能力探测尚未纳入只读协议。
- **SSH 建连闭环原则**:初始连接不能只给 TCP/握手设超时,完整 `connect→auth→request_pty→request_shell` 链路必须有总超时;失败或超时必须返回 `Closed{error}`,不得让 UI 长期停留在连接中。
- `AuthProvider`(密码/口令/keyboard-interactive)由工厂按会话创建;session 层会用 `InteractiveAuthProvider` 包装 GUI provider。GUI provider 先读 vault 中已有密码或 `key:{key_path}` 私钥口令;缺失时 core 发带连接代次的 `AuthChallenge` 给 UI,UI 弹窗采集后用同一代次的 `AuthResponse` 回传，旧代次答案直接丢弃。认证等待期间 `SessionState.auth_challenge` 非空,状态栏显示“等待认证”。同步等待认证答案必须放入 Tokio `block_in_place`，避免多个认证弹窗耗尽 runtime worker；认证答案仍通过独立响应通道回到认证流程,不要混入终端 `Input`。
