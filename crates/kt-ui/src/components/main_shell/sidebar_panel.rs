...
pub(crate) const fn sidebar_header_actions() -> [SidebarHeaderAction; 2] {
    [
...
pub(super) fn render_sidebar_panel(args: SidebarPanelArgs) -> Element {
    ...
    let sidebar_style = format!("--sidebar-width: {:.0}px;", sidebar_width());
    ...
    rsx! {
        aside {
            ...
            class: "{sidebar_class}",
            style: "{sidebar_style}",
            aria_hidden: sidebar_collapsed.to_string(),
            inert: sidebar_inert_attribute(sidebar_collapsed),
...
fn sidebar_inert_attribute(collapsed: bool) -> Option<&'static str> {
    collapsed.then_some("")
}
...
mod tests {
    ...
    #[test]
    fn sidebar_header_actions_only_include_connection_creation_actions() {
        assert_eq!(
    ...
    #[test]
    fn sidebar_inert_attribute_is_absent_when_expanded() {
        assert_eq!(sidebar_inert_attribute(false), None);
        assert_eq!(sidebar_inert_attribute(true), Some(""));
    }
}
