    ...
    use_effect({
        ...
        move || {
            ...
            spawn(async move {
                ...
                let terminal_screen_id_js = format!("{terminal_screen_id:?}");
                let script = format!(
                    r#"
                const terminalId = {terminal_id_js};
                const terminalScreenId = {terminal_screen_id_js};
                ...
                const measure = () => {{
                    ...
                    const screen = document.getElementById(terminalScreenId);
                    const row = screen?.querySelector('.terminal-row');
                        ...
                        div {
                            ...
                            if show_line_numbers && !meta.unused {
                                if meta.is_continuation {
                                    ...
                                    span { class: "terminal-gutter-lineno", "{gutter_line_number(gutter_first_line, &gutter_rows, idx)}" }
                                }
                            }
                        }
                    }
...
/// 返回可见区域中某个逻辑行的编号。自动换行的续行不消耗行号，避免显示为
/// `1`、`-`、`3` 这类跳号。
fn gutter_line_number(first_line: usize, rows: &[RowMeta], row: usize) -> usize {
    first_line
        + rows[..row]
            .iter()
            .filter(|meta| !meta.is_continuation)
            .count()
}
...
fn paste_clipboard_to_terminal(state: Arc<Mutex<AppState>>, session_id: SessionId) {
    ...
    match crate::clipboard::read_text() {
        ...
        Ok(None) => return,
        Err(error) => {
            tracing::warn!("读取系统剪贴板失败，回退 WebView 剪贴板: {}", error);
        }
    }
...
fn terminal_cell_style(cell: &SnapshotCell) -> String {
    ...
    let fg_value = if cell.default_fg {
        "var(--terminal-default-fg)"
    } else {
        fg_color.as_str()
    };
    ...
    let width = if cell.attrs.wide { "2ch" } else { "1ch" };

    format!(
        "color: {fg_value}; background: {bg_value}; display: inline-block; position: relative; width: {width}; font-weight: {font_weight}; font-style: {font_style}; text-decoration: {text_decoration}; opacity: {opacity};"
    )
}
...
mod tests {
    ...
    #[test]
    fn wide_terminal_cells_occupy_two_columns_and_spacers_zero_columns() {
        let wide = SnapshotCell {
            c: '中',
            attrs: kt_core::term::snapshot::CellAttrs {
                wide: true,
                ..Default::default()
        ...
        let ordinary = SnapshotCell {
            c: 'A',
            ..Default::default()
        };

        assert!(terminal_cell_style(&wide).contains("width: 2ch"));
        assert!(terminal_cell_style(&ordinary).contains("width: 1ch"));
        assert!(terminal_spacer_style().contains("width: 0"));
    }
    ...
    #[test]
    fn only_default_foreground_uses_the_theme_color_variable() {
        let default_cell = SnapshotCell {
            c: 'D',
            ..Default::default()
        };
        let explicit_cell = SnapshotCell {
            c: 'R',
            fg: kt_core::term::color::Rgb::new(247, 118, 142),
            default_fg: false,
            ..Default::default()
        };

        assert!(terminal_cell_style(&default_cell).contains("color: var(--terminal-default-fg)"));
        assert!(terminal_cell_style(&explicit_cell).contains("color: rgb(247, 118, 142)"));
    }
    ...
    #[test]
    fn gutter_line_numbers_skip_wrap_continuations() {
        let rows = vec![
            RowMeta::default(),
            RowMeta {
        ...
        assert_eq!(gutter_line_number(1, &rows, 0), 1);
        assert_eq!(gutter_line_number(1, &rows, 2), 2);
    }
