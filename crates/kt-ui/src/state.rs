...
pub struct SessionState {
    ...
    /// 已请求读取终端目录，等待下一条 OSC 7 回传后刷新 SFTP。
    pub terminal_cwd_sync_pending: bool,
...
impl AppState {
    ...
    /// 请求远端 shell 主动通过 OSC 7 上报当前目录，收到后自动刷新 SFTP 浏览目录。
    pub fn sync_sftp_to_terminal_directory(&mut self, id: SessionId) -> Result<(), String> {
        let Some(session) = self.sessions.get_mut(&id) else {
            return Err("会话不存在，无法同步终端目录".to_string());
        };
        if !session.connected {
            return Err("会话已断开，无法同步终端目录".to_string());
        }
        session.terminal_cwd_sync_pending = true;
        session.sftp_error = None;

        if self.manager.send(ToCore::Input {
            id,
            data: terminal_cwd_report_command().into_bytes(),
        }) {
            Ok(())
        } else {
            let message = "终端目录请求无法投递，核心命令队列不可用".to_string();
            if let Some(session) = self.sessions.get_mut(&id) {
                session.terminal_cwd_sync_pending = false;
                session.sftp_error = Some(message.clone());
            }
            Err(message)
        }
    }

    /// 终端目录上报超时后恢复按钮状态并显示可恢复错误。
    pub fn expire_terminal_cwd_sync(&mut self, id: SessionId, message: String) {
        if let Some(session) = self.sessions.get_mut(&id) {
            if session.terminal_cwd_sync_pending {
                session.terminal_cwd_sync_pending = false;
        ...
        match ev {
            ...
            FromCore::Cwd { id, path } => {
                let should_follow = if let Some(sess) = self.sessions.get_mut(&id) {
                    ...
                    std::mem::take(&mut sess.terminal_cwd_sync_pending)
                } else {
                    false
                };
                if should_follow {
            ...
            FromCore::Closed { id, error } => {
                if let Some(sess) = self.sessions.get_mut(&id) {
                    ...
                    sess.terminal_cwd_sync_pending = false;
...
pub fn terminal_cwd_report_command() -> String {
...
mod tests {
    ...
    #[test]
    fn terminal_cwd_event_refreshes_sftp_after_manual_sync_request() {
        let mut app_state = app_state();
        let mut sess = session_state(true);
        sess.terminal_cwd_sync_pending = true;
        let id = sess.id;
        app_state.sessions.insert(id, sess);
        ...
        assert!(!sess.terminal_cwd_sync_pending);
