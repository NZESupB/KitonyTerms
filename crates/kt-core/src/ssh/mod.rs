...
impl SshShell {
    ...
    async fn open_shell_on_handle(
        ...
        Ok(Self {
            handle: Arc::new(authenticated.handle),
    ...
    pub(crate) fn open_exec_channel(
        &self,
        command: &str,
    ) -> impl std::future::Future<Output = Result<russh::Channel<client::Msg>>> + Send + 'static
    {
        let handle = self.handle.clone();
        let command = command.to_string();
        async move {
            let channel =
                tokio::time::timeout(CHANNEL_REQUEST_TIMEOUT, handle.channel_open_session())
                    .await
                    .map_err(|_| SshError::Channel("打开运维查询通道超时(8 秒)".to_string()))?
                    .map_err(|e| SshError::Channel(e.to_string()))?;
            let mut guard = ChannelCloseGuard::new(channel);
            match tokio::time::timeout(CHANNEL_REQUEST_TIMEOUT, guard.exec(true, command.as_str()))
                .await
            {
                Ok(Ok(())) => guard
                    .take()
