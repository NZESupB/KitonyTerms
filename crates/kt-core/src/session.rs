...
/// 批量上传中的一个本地条目。`remote` 是目标远端 POSIX 路径。
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SftpBatchEntry {
...
pub enum SftpRequest {
    ...
    /// 递归上传本地文件/目录，目录条目会先创建，空目录也会保留。
    UploadBatch {
        entries: Vec<SftpBatchEntry>,
        target_dir: String,
        /// 仅允许覆盖用户确认过的远端文件路径；新出现的冲突会停止批次。
...
impl fmt::Debug for SftpRequest {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::UploadBatch {
                ...
                .field("overwrite_paths", &overwrite_paths.len())
                .finish(),
            Self::Mkdir { .. } => f.debug_struct("Mkdir").finish(),
            Self::Remove { is_dir, .. } => {
                f.debug_struct("Remove").field("is_dir", is_dir).finish()
            }
            Self::Rename { .. } => f.debug_struct("Rename").finish(),
        }
    }
}
...
pub enum AuthChallenge {
    ...
    /// Password requested by a privileged operations command. The password is
    /// routed to the matching operation only and is never persisted.
    Sudo {
...
pub enum ToCore {
    ...
    /// Update the local terminal scrollback depth for an active session.
    UpdateTerminalSettings { id: SessionId, scrollback: usize },
    ...
    /// Answer a sudo challenge belonging to one operations request.
    SudoResponse {
        id: SessionId,
        operation_id: OperationId,
        response: AuthResponse,
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            ToCore::Resize { id, cols, rows } => f
                ...
                .field("rows", rows)
                .finish(),
            ToCore::UpdateTerminalSettings { id, scrollback } => f
                .debug_struct("UpdateTerminalSettings")
                .field("id", id)
            ...
            ToCore::CancelOperation { id, operation_id } => f
        ...
        match self {
            ...
            FromCore::AuthChallenge {
                ...
                let (kind, prompt_count) = match challenge {
                    ...
                    AuthChallenge::Sudo { .. } => ("sudo", Some(1)),
                };
...
fn to_core_kind(command: &ToCore) -> &'static str {
    match command {
        ...
        ToCore::UpdateTerminalSettings { .. } => "update_terminal_settings",
        ToCore::Scroll { .. } => "scroll",
        ToCore::Sftp { .. } => "sftp",
        ToCore::StartMonitor { .. } => "start_monitor",
        ToCore::Operations { .. } => "operations",
        ToCore::CancelOperation { .. } => "cancel_operation",
        ...
        ToCore::SudoResponse { .. } => "sudo_response",
...
async fn core_loop(
    ...
    // 路由层跟踪已接受请求，重连替换代次或会话提前关闭也必须提供终态。
    let mut pending_operations: HashMap<(SessionId, OperationId), OperationsDomain> =
        HashMap::new();
    ...
    loop {
        let cmd = tokio::select! {
            Some(event) = session_event_rx.recv() => {
                if session_generation_is_current(&sessions, event.id, event.generation) {
                    ...
                    match &event.event {
                        FromCore::OperationResult { id, operation_id, .. }
                        | FromCore::OperationFailed { id, operation_id, .. }
                            if pending_operations.remove(&(*id, *operation_id)).is_none() => {
                            continue;
                        }
                        _ => {}
                    }
                    if closed {
                        cancel_pending_operations(&mut pending_operations, &tx, event.id).await;
                    }
        ...
        match cmd {
            ToCore::Connect { id, params, pty } => {
                cancel_pending_operations(&mut pending_operations, &tx, id).await;
            ...
            ToCore::UpdateTerminalSettings { id, scrollback } => {
                if let Some(h) = sessions.get(&id) {
                    let _ = h
                        .cmd_tx
                        .send(SessionCmd::UpdateTerminalSettings { scrollback });
                }
            }
            ...
            ToCore::CancelOperation { id, operation_id } => {
                if let Some(domain) = pending_operations.remove(&(id, operation_id)) {
                    let _ = tx
                ...
                if let Some(handle) = sessions.get(&id) {
                    let _ = handle
                        .cmd_tx
                        .send(SessionCmd::CancelOperation { operation_id });
                }
            }
            ToCore::Operations {
                id,
                ...
                if let Some(h) = sessions.get(&id) {
                    if pending_operations.contains_key(&(id, operation_id)) {
                        continue;
                    }
                    pending_operations.insert((id, operation_id), domain);
                    let generation = h.generation;
                    if h.cmd_tx
            ...
            ToCore::SudoResponse {
                ...
                if let Some(h) = sessions.get(&id) {
                    let _ = h.cmd_tx.send(SessionCmd::SudoResponse {
                        operation_id,
                        response,
                    });
                }
...
enum SessionInternal {
    OperationCompleted {
        operation_id: OperationId,
        result: Result<OperationsResult, OperationsError>,
    },
...
impl SessionTask {
    async fn run(mut self) {
        ...
        let mut scrollback = 10_000;
        let mut terminal_cols = self.pty.cols;
        let mut terminal_rows = self.pty.rows;
        ...
        let mut operation_tasks: HashMap<OperationId, OperationTaskHandle> = HashMap::new();
        let mut pending_sudo: HashMap<OperationId, oneshot::Sender<Option<String>>> =
            HashMap::new();
        ...
        loop {
            tokio::select! {
                internal = internal_rx.recv() => {
                    match internal {
                        Some(SessionInternal::OperationCompleted { operation_id, result }) => {
                            pending_sudo.remove(&operation_id);
                            if let Some(handle) = operation_tasks.remove(&operation_id) {
                                send_operation_outcome(&self.out, id, operation_id, handle.domain, result).await;
                            }
                        }
                ...
                cmd = self.cmd_rx.recv() => {
                    match cmd {
                        ...
                        Some(SessionCmd::UpdateTerminalSettings { scrollback: next }) => {
                            scrollback = next.clamp(1_000, 100_000);
                            term.resize(terminal_cols as usize, terminal_rows as usize, scrollback);
                            self.emit_render(&term);
                        }
                        ...
                        Some(SessionCmd::CancelOperation { operation_id }) => {
                            pending_sudo.remove(&operation_id);
                            if let Some(handle) = operation_tasks.remove(&operation_id) {
                                cancel_operation_task(handle, &self.out, id, operation_id).await;
                            }
                        }
                        Some(SessionCmd::Operations { operation_id, request }) => {
                            let domain = request.domain();
                            if operation_tasks.contains_key(&operation_id) {
                                continue;
                            }
                            let command = match remote_ops::command_for_request(&request) {
                                Ok(command) => command,
                                Err(error) => {
                            ...
                            let sudo_response_rx = if remote_ops::request_requires_sudo(&request) {
                                let (tx, rx) = oneshot::channel();
                                pending_sudo.insert(operation_id, tx);
                                Some(rx)
                            } else {
                                None
                            };
                            let opening = shell.open_exec_channel(&command);
                            let out = self.out.clone();
                            let internal_tx = internal_tx.clone();
                            let task = tokio::spawn(async move {
                                let result = match tokio::time::timeout(MONITOR_OPEN_TIMEOUT, opening).await {
                                    Ok(Ok(channel)) => remote_ops::execute_request(id, operation_id, request, channel, out, sudo_response_rx).await,
                                    Ok(Err(_)) => Err(OperationsError::new(OperationsErrorKind::Disconnected, "打开运维查询通道失败")),
                                    Err(_) => Err(OperationsError::new(OperationsErrorKind::Timeout, "打开运维查询通道超时（8 秒）")),
                                };
                                let _ = internal_tx.send(SessionInternal::OperationCompleted { operation_id, result });
                            });
                            operation_tasks.insert(operation_id, OperationTaskHandle { domain, task });
                        }
                        Some(SessionCmd::SudoResponse { operation_id, response }) => {
                            let password = match response {
                                AuthResponse::Answers(mut answers) => answers.drain(..).next(),
                                AuthResponse::Cancel => None,
                            };
                            if let Some(sender) = pending_sudo.remove(&operation_id) {
                        ...
                        Some(SessionCmd::Disconnect) | None => {
                            ...
                            cancel_operation_tasks(&mut operation_tasks, &self.out, id).await;
                            pending_sudo.clear();
        ...
        while let Ok(command) = self.cmd_rx.try_recv() {
            if let SessionCmd::Operations {
                operation_id,
                request,
            } = command
            {
                send_operation_outcome(
                    &self.out,
                    id,
                    operation_id,
                    request.domain(),
...
async fn cancel_pending_operations(
    pending: &mut HashMap<(SessionId, OperationId), OperationsDomain>,
    ...
    for key in cancelled {
        if let Some(domain) = pending.remove(&key) {
            let _ = out
                .send(FromCore::OperationFailed {
                    id,
                    operation_id: key.1,
...
async fn send_operation_outcome(
    out: &SessionEventSender,
    id: SessionId,
    operation_id: OperationId,
    domain: OperationsDomain,
    ...
    let event = match result {
        Ok(result) => FromCore::OperationResult {
            id,
            operation_id,
            domain,
            result,
        },
        Err(error) => FromCore::OperationFailed {
            id,
            operation_id,
            domain,
            error,
...
async fn cancel_operation_task(
    handle: OperationTaskHandle,
    out: &SessionEventSender,
    id: SessionId,
    operation_id: OperationId,
) {
...
async fn cancel_operation_tasks(
    tasks: &mut HashMap<OperationId, OperationTaskHandle>,
    out: &SessionEventSender,
    id: SessionId,
) {
    for (operation_id, handle) in tasks.drain() {
        cancel_operation_task(handle, out, id, operation_id).await;
    }
}
...
mod tests {
    ...
    fn to_core_debug_redacts_sensitive_payloads() {
        ...
        let upload_debug = format!(
            "{:?}",
            ToCore::Sftp {
                id: SessionId(1),
                request_id: SftpRequestId(4),
                req: SftpRequest::UploadBatch {
                    entries: vec![SftpBatchEntry {
                        local: std::path::PathBuf::from("/Users/private/secret.txt"),
                        remote: "/srv/secret.txt".to_string(),
                        is_dir: false,
                    }],
                    target_dir: "/srv".to_string(),
                    overwrite_paths: Vec::new(),
                },
            }
        );
        assert!(upload_debug.contains("entries"));
        assert!(!upload_debug.contains("private"));
        assert!(!upload_debug.contains("secret.txt"));
    }
