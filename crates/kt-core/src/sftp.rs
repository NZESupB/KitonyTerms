...
use crate::session::{
    FromCore, SessionEventSender, SessionId, SftpBatchEntry, SftpEntry, SftpOp, SftpRequest,
    SftpRequestId,
};
...
/// 目录递归上传允许多个文件顺序传输。
const BATCH_TRANSFER_TIMEOUT: Duration = Duration::from_secs(15 * 60);
...
async fn handle_inner(
    ...
    match req {
        ...
        SftpRequest::Upload { local, remote } => {
            upload_file(session, id, request_id, local, remote, true, out).await?;
            let _ = send_sftp_event(
                out,
                FromCore::SftpDone {
                    id,
        ...
        SftpRequest::UploadBatch {
            entries,
            target_dir,
            overwrite_paths,
        } => {
            if entries.is_empty() {
                return Err("上传批次为空".to_string());
            }
            for entry in entries {
                if entry.is_dir {
                    ensure_remote_directory(session, entry).await?;
                } else {
                    upload_file(
                        session,
                        id,
                        ...
                        &entry.local,
                        &entry.remote,
                        overwrite_paths.iter().any(|path| path == &entry.remote),
                        out,
                    )
                    .await?;
                }
...
/// 确保批次中的目录存在；已有目录合并，文件与目录同名则停止批次。
async fn ensure_remote_directory(
    session: &SftpSession,
    entry: &SftpBatchEntry,
) -> Result<(), String> {
    let local_metadata = std::fs::symlink_metadata(&entry.local)
        .map_err(|error| format!("读取本地目录 {} 失败：{error}", entry.local.display()))?;
    if local_metadata.file_type().is_symlink() || !local_metadata.is_dir() {
        return Err(format!("上传源 {} 不再是目录", entry.local.display()));
    }
    match session.metadata(entry.remote.clone()).await {
        Ok(metadata) if metadata.is_dir() => Ok(()),
        Ok(_) => Err(format!("上传目录 {} 与远端文件冲突", entry.remote)),
        Err(_) => session
            .create_dir(entry.remote.clone())
            .await
            .map_err(|error| format!("创建远端目录 {} 失败：{error}", entry.remote)),
    }
}

/// 将一个本地普通文件通过同目录临时文件安全提交到远端。
async fn upload_file(
    ...
    let name = basename(remote);
    let local_metadata = std::fs::symlink_metadata(local)
    ...
    if let Err(error) = commit_uploaded_temp(session, &temp_path, remote, target_exists).await {
